...
def create_filter(body: FilterCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    f = SavedFilter(user_id=user.id, name=body.name, query=body.query, filters=body.filters)
    db.add(f)
    db.commit()
    db.refresh(f)
    return f
...
def delete_filter(filter_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    f = db.exec(select(SavedFilter).where(SavedFilter.id == filter_id, SavedFilter.user_id == user.id)).first()
    if not f:
